async function add1(valores){
    ...
    await firebase.database().ref("rating").child("hooky").child("media").set(novaMedia)
    await firebase.database().ref("rating").child("hooky").child("nota").set(novaNota)
    await firebase.database().ref("rating").child("hooky").child("acumulador").set(novoAcumulador)
...
document.getElementById("star-1").addEventListener("click", function (){
    firebase.database().ref("rating").child("hooky").once("value").then(snapshot => {
        add1(snapshot.val())
    })
})
...
document.addEventListener("DOMContentLoaded", async function (){
    firebase.database().ref("rating").child("hooky").once("value").then(async function(snapshot) {
        const notaText = document.getElementById("nota-text")
