function visitou(visitante){
    ...
    firebase.database().ref("views").child('hooky').set(novaVisita)
}
...
document.addEventListener("DOMContentLoaded", async function (){
    firebase.database().ref("views").child("hooky").once("value").then(async function(snapshot) {
        const url = window.location.href
        if(url.slice(0,48) == "https://scanaticos.site/projetos/hooky/capitulos"){
